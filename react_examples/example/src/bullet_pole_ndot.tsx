...
export function Bullet_Pole_ndot() {
  ...
  useEffect(() => {
    if (loadingCountRef.current === 0) {
      ...
      const createSoftGeometry = function (tX, tY, tZ) {
        const sizeX = 3;
        const sizeY = 3;
        const sizeZ = 8;
        ...
        numFaces    += 2 * 2 * ( ( numPointsX - 1 ) * ( numPointsY - 1 ) + ( numPointsX - 1 ) * ( numPointsZ - 1 ) + ( numPointsY - 1 ) * ( numPointsZ - 1 ) );
        ...
        var indices_ex = new ( exNumFaces * 3 > 65535 ? Uint32Array : Uint16Array )( exNumFaces * 3 );
        ...
        for ( var p = 0, k = 0; k < numPointsZ; k++ ) {
            for ( var j = 0; j < numPointsY; j++ ) {
                for ( var i = 0; i < numPointsX; i++ ) {
                    ...
                    if ( k != numPointsZ - 1 && i < numPointsX - 1 && j < numPointsY - 1 ) {
                    ...
                    // else if (k != numPointsZ - 1 && i < numPointsX - 1 && j < numPointsY - 1) {
                    //     var faceIndex = exface_num * 3;

                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 0 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 3 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 1 ];

                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 0 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 2 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 3 ];
                    //     exface_num += 2;
                    // }
                    ...
                    if ( j != numPointsY -1 && i < numPointsX - 1 && k < numPointsZ - 1 ) {
                    ...
                    // else if (j !== numPointsY -1 && i < numPointsX - 1 && k < numPointsZ - 1){
                    //     var faceIndex = exface_num * 3;

                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 0 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 5 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 4 ];
                    ...
                    // else if (i != numPointsX - 1 && j < numPointsY - 1 && k < numPointsZ - 1) {
                    //     var faceIndex = exface_num * 3;

                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 0 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 6 ];
                    //     indices_ex[ faceIndex++ ] = p + indexFromOffset[ 2 ];
        ...
        console.log('numFaces', numFaces, 'numFacesAdded', numFacesAdded, 'exface_num', exface_num);
        ...
        // scene.add(volume);
        createSoftVolume( volume.geometry, 200, 0.95, 0.01, 0.9, 0.9, 1, material );

        // console.log('numFaces', numFaces, 'exface_num', exface_num);
        // console.log('Length of indices:', indices.length);
        // console.log('Length of indices_ex:', indices_ex.length);
        // const bufferGeom_ex = new THREE.BufferGeometry();
        // bufferGeom_ex.setIndex( new THREE.BufferAttribute( indices_ex, 1 ) );
        // bufferGeom_ex.setAttribute( 'position', new THREE.BufferAttribute( vertices, 3 ) );
        // bufferGeom_ex.setAttribute( 'normal', new THREE.BufferAttribute( normals, 3 ) );
        // bufferGeom_ex.translate( tX, tY, tZ );
        // const material_ex = new THREE.MeshBasicMaterial( { color: 0xFF0000, wireframe: true } )
        // const volume_ex = new THREE.Mesh( bufferGeom_ex, material_ex);
        // scene.add(volume_ex);
        // createSoftVolume( volume_ex.geometry, 10, 0.95, 0.01, 0.9, 0.9, 1, material_ex );
        ...
        stiffness: number=0.85, constraints: number=3) 
      {
        ...
        //   "/models/simulation_base_cylinder_thicken.glb",
          (gltf) => {
      ...
      const createObjects = function () {
        ...
        // createSoftbody({x: -3, y: 1.12, z: 0}, 50, 1, 0.95, 0.95, 0.05, 10);
        createSoftbody({x:  0, y: 1.12, z: 0}, 50, 1, 0.95, 0.95, 0.05, 10);
        // createSoftbody({x:  3, y: 1.12, z: 0}, 50, 1, 0.95, 0.95, 0.05, 10);

        // createSoftGeometry(0, 1.6, 0);
    ...
    const createGImpactCollision = function(mesh, scale) {
      ...
      const dropCone = function ( mass, pos ) {
        ...
        const ballShape = new Ammo.btConeShape( ballRadius*2, ballHeight*2 );
        ...
        gRigidBodies.push(ballBody);

        coneList.push(ball);

        ballBody.setLinearVelocity( new Ammo.btVector3( 0,-15, 0) );
      ...
      const initInput = function () {
        ...
        window.addEventListener('keydown', function(event) {
          ...
          if (event.key === '1') {
            ...
            // dropCone(2, new THREE.Vector3(-2.6, 5, -2));
            dropCone(2, new THREE.Vector3(0.4, 5, -2));
            // dropCone(2, new THREE.Vector3(3.4, 5, -2));
          }
          else if (event.key === '3') {
            dropCone2(1, pos);
          }
      ...
      const checkCollisionAndApplyDeformation = function () {
        if ( gSoftBodies.length === 0 || gRigidBodies.length === 0) return;
        const Ammo = window.ammoInstance;
        
        const softBody = gSoftBodies[0];
        // const rigidBody = gRigidBodies[0];
        // physicsWorld.contactPairTest(softBody, rigidBody, new Ammo.ConcreteContactResultCallback());

        const impactThreshold = 0.01
        let links = softBody.get_m_links();

        const link = links.at(impact_id);
        console.log(link.get_m_rl(), link.m_n_at(0).get_m_x().length() - link.m_n_at(0).get_m_q().length());

        let num_impacts = 0;
        for (let i = 0; i < links.size(); i++) {
            const link = links.at(i);
            const node0 = link.m_n_at(0);
            const node1 = link.m_n_at(1);
            const p0 = node0.get_m_x();
            const p1 = node1.get_m_x();
            const q0 = node0.get_m_q();
            const q1 = node1.get_m_q();
            ...
            const dist_p = Math.sqrt(px*px + py*py + pz*pz);
            const dist_q = Math.sqrt(qx*qx + qy*qy + qz*qz);

            if (Math.sqrt((dist_p - dist_q)*(dist_p - dist_q)) > impactThreshold ) {
                impact_id = i;
                num_impacts++;
                node0.set_m_q(node0.get_m_x());
                node1.set_m_q(node1.get_m_x());
                link.set_m_rl(0);
                link.set_m_bbending(0);
                link.set_m_c0(0);
                link.set_m_c1(0);
                link.set_m_c2(0);
                // break;
      ...
      const updatePhysics = function ( deltaTime: number ) {
        ...
        for ( var i = 0, il = rigidBodies.length; i < il; i++ ) {
          ...
          checkCollisionAndApplyDeformation();
      ...
      const mergeVertices2 = function ( geometry: any, precisionPoints: any = 4 ) {
        ...
        const uvArray = uv !== undefined ? uv.array : undefined;
      ...
      const createSoftVolume = function ( geometry,  
        ...
        gSoftBodies.push( volumeSoftBody );


        var sbConfig = volumeSoftBody.get_m_cfg();
        console.log("node:", volumeSoftBody.get_m_nodes().size());
        console.log("link:", volumeSoftBody.get_m_links().size());
        console.log("face:", volumeSoftBody.get_m_faces().size());

        sbConfig.set_viterations( 50 );
        sbConfig.set_piterations( 50 );
        // sbConfig.set_diterations( 50 );
        sbConfig.set_citerations( 20 );
