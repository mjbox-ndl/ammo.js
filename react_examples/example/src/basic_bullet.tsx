...
export function Basic_Bullet() {
  ...
  useEffect(() => {
          ...
          (gltf) => {
            gltf.scene.traverse((child) => {
                if (child.type == 'Mesh') {
                    ...
                    // createSoftVolume(geometry, 15, 250, 0.5, child.material );

                    createSoftVolume( child.geometry, 15, 250, 0.5, child.material );
    ...
    const createGImpactCollision = function(mesh, scale) {
          ...
          if (event.key === '1') {
            ...
            gltfLoader.load(
              ...
              (gltf) => {
                ...
                gltf.scene.position.set(0.5, 6, 0);
                gltf.scene.traverse((child) => {
                    if (child.type == 'Mesh') {
                      ...
                      const body = createRigidBody( child, trimesh, 0.005, pos, quat );
            ...
            var ball = new THREE.Mesh( new THREE.ConeGeometry( ballRadius, 1, 15 ), Material );
      ...
      const createSoftVolume = function ( geometry,  mass: number, pressure: number, kDP: number, material: any = null ) {
        const Ammo = window.ammoInstance;
        ...
        volumeSoftBody.setTotalMass( 20, true );
